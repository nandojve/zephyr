...
/* typedef is used to checkpatch identify the type */
typedef Usbc Usbc_t;
...
struct udc_sam_usbc_data {
	 ...
	 *
	 * Placed before DMA buffers to avoid potential corruption from
	 * USB DMA writes to the setup buffer.
	 */
	 ...
	 * Must be the last field - DMA may overrun the buffer.
	 */
...
static void sam_usbc_isr_sta_dbg(Usbc_t *base, uint8_t ep_idx, uint32_t sr)
{
...
static void sam_usbc_ep_isr_sta(Usbc_t *base, uint8_t ep_idx)
{
...
static ALWAYS_INLINE void sam_usbc_freeze_clk(Usbc_t *const base)
{
	base->USBCON |= USBC_USBCON_FRZCLK;
}

static ALWAYS_INLINE void sam_usbc_unfreeze_clk(Usbc_t *const base)
{
	base->USBCON &= ~USBC_USBCON_FRZCLK;
...
#define UDC_SAM_USBC_DEVICE_DEFINE(n)						      \
	...
	static const struct udc_sam_usbc_config udc_sam_usbc_config_##n = {	      \
		.base = (Usbc_t *)DT_INST_REG_ADDR(n),				      \
		.dt = sam_usbc_dt_##n,						      \
