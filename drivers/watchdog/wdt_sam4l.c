				  ...
				  uint32_t time)
{
	...
	for (int scale = 7; scale <= 31; ++scale) {
		uint32_t timeout = (BIT(scale + 1) * 1000ull) / wdt_clk;
...
/**
 * @brief Calculate the both Banned and Prescale Select timeout to be installed
 * in the watchdog timer.
 *
 * The config->min value will define the banned timeout. The prescaller timeout
				     ...
				     const struct wdt_timeout_cfg *config)
{
	...
	if (config->flags & WDT_FLAG_RESET_CPU_CORE) {
		LOG_ERR("The SAM4L watchdog do not support reset CPU core");
		return -ENOTSUP;
	}
